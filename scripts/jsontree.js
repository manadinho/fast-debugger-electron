...
var jsonTree = (function() {
     ...
     * @return {Node}
     */
    function Node(label, val, isLast, notJsonData = false) {
        var nodeType = utils.getType(val);
        
        if (nodeType in Node.CONSTRUCTORS) {
            return new Node.CONSTRUCTORS[nodeType](label, val, isLast, notJsonData);
        } else {
            throw new Error('Bad type: ' + utils.getClass(val));
        }
    }
     ...
     */
    function _NodeSimple(label, val, isLast, notJsonData) {
        ...
        var self = this,
            ...
            template = function(label, val) {
                ...
                if(notJsonData) {
                    var str = '\
                    <span class="jsontree_value-wrapper" style="margin-top:10px">\
                        <span class="jsontree_value jsontree_value_' + self.type + '">' +
                            val +
                        '</span>' +
                        (!isLast ? ',' : '') + 
     ...
     * @param isLast {boolean} - true if node is last in list of parent childNodes
     */
    function NodeNumber(label, val, isLast, notJsonData) {
        this.type = "number";
    
        _NodeSimple.call(this, label, val, isLast, notJsonData);
    }
     ...
     * @param isLast {boolean} - true if node is last in list of parent childNodes
     */
    function NodeString(label, val, isLast, notJsonData) {
        this.type = "string";
    ...
    /** FAST DEBUGGER CUSTOM METHOD TO GET TIME */
    function generateTime() {
        const date = new Date();
        return `${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`
    }

    /** FAST DEBUGGER CUSTOM METHOD TO GENERATE LINK TO OPEN FILE IN VSCODE */
     ...
     */
    function Tree(jsonObj, domEl, flag, path, notJsonData) {
        ...
        this.loadData(jsonObj, flag, path, notJsonData);
        this.appendTo(domEl);
    }
    ...
    Tree.prototype = {
         ...
         */
        loadData : function(jsonObj, flag, path, notJsonData) {
            ...
            this.rootNode = new Node(null, jsonObj, 'last', notJsonData);
            ...
            if(path) {
                ...
                <div class="logo-section" style="margin-botton:10px"> 
                ...
                <div class="file-path-wrapper"><a href="${link}" title="Open in Editor" class="file-path" >${path.split('/').pop()}</a></div>
                `;
            }
    ...
    return {
         ...
         * @returns {Tree}
         */
        create : function(jsonObj, domEl, flag = null, path = null, notJsonData = false) {
            return new Tree(jsonObj, domEl, flag, path, notJsonData);
        }
    };
})();
